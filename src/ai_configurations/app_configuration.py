...
class AppConfiguration:
    ...
    async def _get_all_data(self) -> Any:
        ...
        try:
            config = await self.config_client.json().get(self.config_key)
    ...
    async def _get_json_data(self, path: str) -> Any:
        """Helper method to safely get JSON data from Redis"""
        try:
            data = await self.config.json().get(self.config_key, path)
            return data
        except Exception as e:
