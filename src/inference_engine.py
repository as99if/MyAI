...
class InferenceEngine:
    def __init__(self, config: Dict[str, Any], vector_memory_service, conversation_store_service):
        """
        Initialize the inference engine with both memory services
        
        Args:
            config: Configuration dictionary
            vector_memory_service: Service for vector-based memory and semantic search
            conversation_store_service: Service for conversation history
        """
        self.api = config["api"]
    ...
    async def _get_relevant_context_async(self, prompt: str) -> str:
        """Asynchronous method to get relevant context"""
        loop = asyncio.get_event_loop()
        return await loop.run_in_executor(
            self.executor, 
    ...
    def _optimize_prompt(self, prompt: str) -> str:
        """Synchronous prompt optimization method"""
        try:
    ...
    async def advanced_async_inference(self, prompt: Union[str, dict], image: Optional[Image.Image] = None, encoded_image: str = None):
        """
        Advanced asynchronous inference with more granular async steps
        """
        try:
    ...
    def _initialize_llm(self):
        ...
        if self.api == "openai":
            self.llm = OpenAI(model_name="text-davinci-003", openai_api_key=self.openai_api_key)
        elif self.api == "groq":
            self.llm = ChatGroq(model="llama-3.3-70b-specdec",api_key=self.groq_api_key, cache=True)
    ...
    def _initialize_prompt_optimizer(self):
        ...
        if self.api == "openai":
            dspy.settings.configure(lm=dspy.OpenAI(api_key=self.openai_api_key))
        elif self.api == "groq":
    ...
    def _get_relevant_context(self, prompt: str) -> str:
        ...
        recent_conversations = self.conversation_store_service.get_recent_conversations(limit=10)
        ...
        if recent_conversations:
            ...
            for conv in recent_conversations:
                for msg in conv.get('messages', [])[-2:]:  # Only last 2 messages per conversation
        ...
        if vector_context:
            ...
            memory_context += "\n".join([mem.page_content for mem in vector_context])
    ...
    def _save_interaction(self, prompt: str, response: str, metadata: Dict = None):
        ...
        self.conversation_store_service.save_chat_messages(chat_messages, enhanced_metadata)
    ...
    def infer(self, prompt: Union[str, dict], image: Optional[Image.Image] = None, encoded_image: str = None):
        ...
        try:
            ...
            # Handle inference based on input type
            if image or encoded_image:
                response = self._handle_image_inference(final_prompt, image, encoded_image)
            else:
    ...
    def _handle_text_inference(self, prompt_text: str):
        ...
        try:
            ...
            print(f"Conversation chain failed: {e}. Falling back to direct LLM call.")
