...
async def tests():
    ...
    memory_processor = MemoryProcessor(config)
    await memory_processor.connect()
    await memory_processor.delete_all()
    ...
    await memory_processor.ingest_conversation_heistory(sample_conversation_history)

    inference_engine = InferenceEngine(
        config=config, conversation_history_service=memory_processor)
